## Start-Demo.ps1
## Clay copied from https://www.powershellgallery.com/packages/NameIT/1.8.1/Content/start-demo.ps1
## and modified it just a bit
##################################################################################################
...
#                         $dur.Hours, $dur.Minutes, $dur.Seconds, $($_Lines[$_i])
    $RawUI.WindowTitle = "{3} $(if($dur.Hours -gt 0){'{0}h '})$(if($dur.Minutes -gt 0){'{1}m '}){2}s " -f 
        $dur.Hours, $dur.Minutes, $dur.Seconds, $file
...
"@
        ...
        "p" { # Previous
          ...
          $_i--
          while ($_lines[$($_i)].StartsWith("#") -or [string]::IsNullOrWhiteSpace($_lines[$($_i)]))
          {   $_i--
          }
          $_i-- # back a line, we're gonna step forward when we loop
        }
        ...
        "v" { # View Source
            $_lines[0..($_i-1)] | Write-Host -Fore Yellow 
            $_lines[$_i]        | Write-Host -Fore Green
            $_lines[($_i+1)..$_lines.Count] | Write-Host -Fore Yellow 
            $_i-- # back a line, we're gonna step forward when we loop
        }
        "t" { # Time Check
             $dur = [DateTime]::Now - $_StartTime
